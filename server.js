...
wss.on("connection", ws => {
  ws.on("message", msg => {
    ...
    if(data.type==="dice"){
      ...
      // События
      if(event.startsWith("+")) hypeChange = parseInt(event.slice(1)) + lapsBonus;
      else if(event.startsWith("-10")) hypeChange = -10 + lapsBonus;
      else if(event==="start") hypeChange = 15 + lapsBonus;
        ...
        hypeChange = roll<=3?-5+lapsBonus:5+lapsBonus;
      }
